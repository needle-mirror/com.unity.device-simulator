...
A directory where custom devices are placed can be specified in `Preferences -> Device Simulator -> Device Directory`. Device directory can be outside of a Unity project.

Device files that are shipped together with Device Simulator can be found inside the package in `Editor/.DeviceDefinitions` folder.

Device files must have a `.device.json` extension.

Internally we use the following `DeviceInfo` class to serialize and deserialize device files:
```csharp
[Serializable]
